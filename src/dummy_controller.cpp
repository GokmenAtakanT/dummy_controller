...
DummyController::DummyController() : Node("dummy_controller")
{
    ...
    declare_parameter<double>("PurePursuit.min_ld", 2.0);
...
double DummyController::lateralControl(nav_msgs::msg::Path::SharedPtr path, nav_msgs::msg::Odometry::SharedPtr odom)
{
    ...
    auto delta_distance=std::sqrt( std::pow(delta_x,2)+ std::pow(delta_y,2));
    alfa_ = std::atan2(delta_y, delta_x) - yaw_;

    //goal_reached_=(path->poses.size()-5==index_)? true:false;
    goal_reached_=(delta_distance<0.5)? true:false;
    path_received_=(goal_reached_) ? false:true;

    //RCLCPP_INFO(this->get_logger(), "Alfa_ ====> %f", alfa_);

    //RCLCPP_ERROR(this->get_logger(), "STEER_ANGLE ====> %f", std::atan2(2 * wheel_base_ * std::sin(alfa_) , ld));

    return -std::atan2(2 * wheel_base_ * std::sin(alfa_) , ld);
}
...
{
    backlash_=(abs(alfa_)>M_PI/2)? true:false;
    //auto drive_=(backlash) ? -longitudinalControl():longitudinalControl();
    steer_=(backlash_) ? -3*lateralControl(following_path_, vehicle_odom_):lateralControl(following_path_, vehicle_odom_);
    RCLCPP_INFO(this->get_logger(), "backlash_ ====> %d   alfa_ ====> %f    goal_reached_====> %d", backlash_, alfa_, goal_reached_);
    ...
    ego_vehicle_commands.steer = (path_received_ && !goal_reached_) ? steer_ : 0.0;
    ego_vehicle_commands.brake = (!goal_reached_)? 0.0:1;
    ego_vehicle_commands.hand_brake =  false;
    ego_vehicle_commands.reverse = backlash_;
